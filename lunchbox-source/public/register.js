...
form.addEventListener('submit', async event => {
  ...
  try {
    const response = await fetch('/grocery-api/customers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    window.setTimeout(() => window.location.assign('/grocery/'), 900);
