...
function Storefront() {
  ...
  const signedInCustomerId = customer?.customer_id;
  ...
  useEffect(() => { fetch(endpoint('/auth/me')).then(response => response.ok ? response.json() : null).then(data => setCustomer(data)).catch(() => setCustomer(null)); }, []);
  useEffect(() => { if (!signedInCustomerId) { setCart([]); setCartReady(true); return; } setCartReady(false); fetch(endpoint('/cart')).then(response => response.ok ? response.json() : { items: [] }).then(data => setCart(Array.isArray(data.items) ? data.items : [])).finally(() => setCartReady(true)); }, [signedInCustomerId]);
  ...
  useEffect(() => { if (signedInCustomerId && cartReady) fetch(endpoint('/cart'), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: cart }) }).catch(() => undefined); }, [cart, signedInCustomerId, cartReady]);
  ...
  const logout = async () => { await fetch(endpoint('/auth/logout'), { method: 'POST' }); setCustomer(null); window.location.assign('/app/'); };
  const chooseLocation = () => { if (!customer) window.location.assign('/register'); else setLocationOpen(true); };
  const saveLocation = (updated: Customer) => { setCustomer(updated); setLocationOpen(false); };
  ...
  return <div className="min-h-screen bg-[#f7faf6] pb-22 text-[#18251d]">
    <header className="sticky top-0 z-20 border-b border-[#e6ece2] bg-white/95 px-4 py-3 backdrop-blur md:px-8">
      <div className="mx-auto flex max-w-7xl items-center gap-2 sm:gap-3">
        <a className="flex shrink-0 items-center gap-2 text-xl font-black tracking-tight text-nelture-700" href="/app"><img className="h-9 w-9 rounded-xl object-cover sm:h-10 sm:w-10" src="/nelture-logo.png" alt="Nelture" /><span className="hidden sm:inline">Nelture<span className="text-[#9fc45f]">.ai</span></span></a>
        ...
        <label className="ml-auto flex h-10 min-w-0 max-w-xl flex-1 items-center gap-2 rounded-xl bg-[#f4f7f2] px-2.5 sm:h-11 sm:px-3 focus-within:ring-2 focus-within:ring-nelture-500"><Search size={18} className="shrink-0 text-slate-500" /><input value={query} onFocus={() => setTab('search')} onChange={e => { setQuery(e.target.value); setTab('search'); }} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Search groceries" /></label>
        ...
        <button aria-label="Notifications" className="hidden h-10 w-10 place-items-center rounded-xl hover:bg-nelture-50 sm:grid"><Bell size={20}/></button>
    ...
    <main className="mx-auto max-w-7xl px-4 py-5 md:px-8">
      ...
      {tab === 'orders' && <Cart customer={customer} cart={cart} total={cartTotal} onChange={updateQuantity} onClear={() => setCart([])} />}
...
function ProductStrip({ products, loading = false, emptyText, onAdd }: { products: Product[]; loading?: boolean; emptyText: string; onAdd: (p: Product) => void }) { const [added, setAdded] = useState<Record<string, boolean>>({}); if (loading) return <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{[1,2,3,4,5].map(x => <div key={x} className="h-72 animate-pulse rounded-2xl bg-white"/>)}</div>; if (!products.length) return <div className="rounded-3xl border border-dashed border-[#ccdbca] bg-white px-6 py-12 text-center"><PackageOpen className="mx-auto text-nelture-500" size={34}/><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-600">{emptyText}</p></div>; return <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{products.map(product => <article key={product.id} className="rounded-2xl border border-[#e5ece2] bg-white p-3 shadow-sm"><div className="grid aspect-square place-items-center rounded-xl bg-[#f7faf6] text-xs text-slate-400">{product.imageUrl ? <img className="h-full w-full object-contain" src={product.imageUrl} alt={product.name}/> : 'Product image'}</div><p className="mt-3 truncate text-sm font-bold">{product.name}</p><p className="mt-1 text-xs text-slate-500">{product.brand} · {product.unit}</p><div className="mt-3 flex items-end justify-between gap-2"><p className="text-sm font-black">{money.format(product.price)}</p><button onClick={() => { onAdd(product); setAdded(current => ({ ...current, [product.id]: true })); }} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${added[product.id] ? "border-nelture-600 bg-nelture-50 text-nelture-700" : "border-nelture-600 text-nelture-700"}`}>{added[product.id] ? "Added to cart" : "ADD"}</button></div></article>)}</div>; }
function Cart({ customer, cart, total, onChange, onClear }: { customer: Customer | null; cart: CartLine[]; total: number; onChange: (id: string, change: number) => void; onClear: () => void }) {
  ...
  return <div><div className="flex items-center justify-between gap-3"><SectionTitle title="Your cart"/>{cart.length > 0 && <button onClick={onClear} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50">Delete cart</button>}</div>{cart.length === 0 ? <div className="mt-4 rounded-3xl bg-white p-12 text-center"><ShoppingBag className="mx-auto text-nelture-500" size={36}/><p className="mt-3 font-bold">Your cart is waiting</p><p className="mt-1 text-sm text-slate-500">{message || 'Add groceries from the catalogue to get started.'}</p></div> : <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_340px]"><div className="space-y-3">{cart.map(line => <div key={line.id} className="flex items-center gap-3 rounded-2xl bg-white p-4"><div className="grid h-15 w-15 shrink-0 place-items-center overflow-hidden rounded-xl bg-nelture-50 text-xs text-slate-400">{line.imageUrl ? <img src={line.imageUrl} alt={line.name} className="h-full w-full object-contain" /> : 'Item'}</div><div className="min-w-0 flex-1"><p className="truncate font-bold">{line.name}</p><p className="text-xs text-slate-500">{line.unit}</p><p className="mt-1 text-sm font-bold">{money.format(line.price)}</p></div><div className="flex items-center rounded-lg border border-nelture-600"><button onClick={() => onChange(line.id, -1)} className="p-2 text-nelture-700"><Minus size={15}/></button><span className="w-7 text-center text-sm font-bold">{line.quantity}</span><button onClick={() => onChange(line.id, 1)} className="p-2 text-nelture-700"><Plus size={15}/></button></div><button onClick={() => onChange(line.id, -line.quantity)} className="text-xs font-semibold text-red-600 hover:underline">Remove</button></div>)}</div><aside className="h-fit rounded-2xl bg-white p-5 shadow-sm"><p className="font-bold">Bill details</p><div className="mt-4 flex justify-between text-sm"><span>Item total</span><b>{money.format(total)}</b></div><div className="mt-2 flex justify-between text-sm"><span>Delivery</span><span className="text-nelture-600">Calculated at checkout</span></div><button onClick={checkout} disabled={submitting} className="mt-5 w-full rounded-xl bg-nelture-600 py-3 text-sm font-bold text-white disabled:opacity-60">{submitting ? 'Placing order…' : 'Proceed to checkout'}</button>{message && <p className="mt-3 text-sm text-red-600">{message}</p>}</aside></div>}</div>;
}
