...
async function mirrorProduct(productId, product) {
  ...
  await rows(`UPDATE ${table('products')} SET price_history = ARRAY(SELECT AS STRUCT rate_id, price, mrp, recorded_at, source FROM ${table('product_price_history')} WHERE product_id=@product_id ORDER BY recorded_at) WHERE product_id=@product_id`, { product_id: productId });
